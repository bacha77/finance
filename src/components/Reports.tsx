                ...
                {activeTab === 'overview' && (
                    <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem', marginBottom: '3rem' }}>
                            {[
                                { label: 'Monthly Net', val: metrics.net, up: metrics.net >= 0, icon: TrendingUp },
                                { label: 'All-Time Funds', val: metrics.totalAssets, up: true, icon: Landmark },
                                { label: 'Monthly Income', val: metrics.income, up: true, icon: ArrowUpRight },
                                { label: 'Monthly Expenses', val: metrics.expenses, up: false, icon: ArrowDownRight }
                            ].map((c, i) => (
